...
line-for-line at a fixed offset of 10,832 before anything was cut, and
re-verified on the checkout the cut was made from). Zero lines differed between
the two copies, so no supersede question arose and no correction could have been
lost — that was checked first precisely because a diverging line might have been
the only copy of a real correction.

ONE region was NOT duplicated and decided WHICH copy to delete: the 51-line
**2026-08-01** record (the PowerShell-5.1 parse defect) appeared exactly once,
stranded BETWEEN the two copies. The dated markers ran 08-06 … 08-03 and then
jumped straight to 07-26 — 08-01 was missing from the timeline. Deleting the
FIRST copy restores it to its chronological slot; deleting the second would have
left it orphaned after the PR-history table, and deleting "everything after the
first copy" would have destroyed the only copy of it.

After: 21,958 → 11,177 lines; `## Current Status` and `## PR history` each
...
Verify-first answer: **no**. There was no realtime layer of ANY kind — no
WebSocket/SSE/SignalR in `src/` or `server/`, no react-query/SWR, no focus or
visibility revalidation; the only two `setInterval`s in the codebase are
display clocks. Every screen was fetch-on-mount (`[]` / `[patientId]`), with
`reload()` firing only after the SAME user's own write in the same tab.
Navigation re-mounts and re-reads, so the failure is not "stale until
re-login" — it is stale **while sitting still**: the ward monitor and the
chart left open through a shift. Two findings were worse than they looked:
the bed board's severity dots come from `useDerivedSeverities` keyed on the
SET of patient ids, and because the bed list never re-read the set could never
change — the dots were computed ONCE at mount and never again, and
`critCount` + the Critical filter derive from that same frozen map, so a newly
critical patient was ABSENT from the Critical view rather than mis-coloured;
and the board's sidebar rendered the literal `Sync: live` while nothing
re-read at all. BUILT (steps 1-2): (1) `<DataAge>` on every board and patient
screen — `Live · HH:MM` / `Not updating · last HH:MM` / `As of HH:MM` /
`Not read`, never green (green is EARNED, 01 Design System), with TWO chips
wherever a polled read sits beside an unpolled one so the fresher number
cannot vouch for the staler; `Sync: live` deleted. (2) `usePollTick`
(`LIVE_POLL_MS` 20s) on the cheap single-request list reads only — bed census,
both worklists, alerts — paused while the document is hidden and catching up
immediately on return. Scores stay mount-computed on purpose:
`fetchPatientScores` costs FIVE requests per patient fanned out per bed
(~100 requests per board mount on a 20-bed unit), so polling them needs the
server-side move first. VERIFIED rendered with a deterministic clock
(Chromium `page.clock`): polls while visible PASS · **pauses while hidden**
PASS · catches up on return PASS · says "Not updating" when quiet PASS · right
chip kind on all 9 screens (8/8 sweep + nurse worklist under a nurse account).
Two defects caught by that verification and fixed: an unreadable source
rendered `loading…` forever (indistinguishable from slow), and Mission Control
initially took its age from the score fetch, reading `Not read` whenever only
the observation domain was unreachable though the rest of the chart had
loaded. 🔴 RECORDED FOR STEP 3 (server-side severity, NOT built): the ported
engine must be re-validated **value-for-value** against the current client
engine before it is trusted — the score-lock discipline from PR #154 makes
this a validation exercise, not a code move (input scopes preserved exactly:
NEWS2 = full chart, SOFA = open encounter's chart else full, plus labs /
orders / encounter weight). Step 4 (SSE/SignalR push) also deferred; `<DataAge>`
stays permanently either way, because a push channel that silently drops is
the same failure in a new costume. Honest limit: no multi-device test against
a live server has been run — the above was verified in one browser against
demo data. Design record: `LIVE_DATA_DESIGN.md`; rule in 03.
