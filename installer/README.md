...
- 🔴 **The updater's EXIT CODES were EXECUTED as real processes, not reviewed.** `installer\test-update-exitcodes.ps1` (**28/28**) spawns child processes, dot-sources the **real** `Fail` / `FailBetweenStates` / `FailRolledBack` definitions and asserts the **real** process exit code — including the shape that actually ships, `FailRolledBack` called from **inside** the rollback `try{}` whose `catch{}` prints the CRITICAL between-states message. It also asserts the `.iss` ↔ `.ps1` argument contract (`-FallbackInstallDir` present, `-InstallDir` absent, log-path relay wired), because the install-dir defect lived in the gap *between* two files that each read correctly alone. **Teeth measured, not claimed:** against the pre-fix code the suite fails, and with only the `-ErrorAction Continue` fix reverted it reports `expected 2, got 1` and `expected 3, got 99` — the second being the catch firing, i.e. a *successful* rollback reported as a catastrophe. Stated limit: codes 2 and 3 cannot be reached **end-to-end** without a live `AuroraServer`, a real Postgres and a half-applied swap, so the helpers are proven and their call sites stay engineer-proven (checklist items 16/17 below).

- 🔴 **The updater's HAPPY path had never run, and did not work (found 2026-08-06, first field run past package verification).** `aurora-update.ps1` formatted psql's stderr with an inline `((Get-Content -Raw $headErr) -replace '\s+',' ').Trim()`. A **successful** psql writes nothing to stderr, so `2>$headErr` leaves a **zero-byte** file, `Get-Content -Raw` emits **no objects**, and `-replace` on that returns an empty `System.Object[]` with no `.Trim()` — terminating under `EAP=Stop`. **No update could ever have completed on any machine.** Fixed with a pure `ConvertTo-SingleLine` (the `[string]` cast is the fix); the same class was killed in `aurora-autowire.ps1` (no `AI_PROVIDER` line) and `build.ps1` (a silent empty commit yields a package whose `/healthz` says `dev`, so its health check can never pass — `-UpdateOnly` now refuses to build one). **Stated limit on the test, because it matters:** the hazard is `AutomationNull`, which PowerShell converts to a plain `$null` at parameter-binding time, so a unit test that *calls* the helper **cannot** reproduce it and stays green with the fix reverted (verified). The teeth are therefore a **static lint** over every shipping `installer\*.ps1`, pinned non-vacuous against the two real historical lines, plus a child-process proof that the inline form throws. Reinstating both defects fails the suite with 2 named offenders.

- 🔴 **Every wizard-facing message is now derived from the exit code.** A version-skew refusal used to exit **0**, which `aurora-update.iss` maps to an empty branch — so a downgrade, or a re-run of an already-installed package, ended on *"The Aurora update has been applied."* It now exits 1. `Start-Service` and `Stop-Service` were outside any `try`, so a new build that would not start died with PowerShell's exit 1 (*"Aurora is exactly as it was"*) **after** the swap, with the service down; the start path now falls through to the health loop and the honest rollback. The finished page asserted success unconditionally on every path and cited a log path that does not exist on an installed machine; it now branches on the code, with an explicit "the updater never ran" state (Pascal zero-initialises, and 0 was the success code). Four static gates in `test-update-exitcodes.ps1` hold this contract.
