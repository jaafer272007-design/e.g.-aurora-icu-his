...
static class VocabApi
{
    ...
    {
        MapVocab(app, "dispositions", "dispositions.manage", "disposition",
        ...
        MapVocab(app, "isolation-types", "isolation.manage", "isolation type",
        ...
        MapVocab(app, "shifts", "shifts.manage", "shift",
    ...
    static string? ValidateCodeLabel(string? rawCode, string? rawLabel, out string code, out string label)
    {
        ...
        if (!System.Text.RegularExpressions.Regex.IsMatch(code, "^[a-z0-9_]{2,40}$"))
            return "code must be 2-40 lowercase letters, digits or underscores (a permanent identifier, e.g. 'higher_care')";
        if (label.Length == 0) return "label is required";
        if (label.Length > 60) return "label exceeds 60 characters";
        return null;
    }

    static void MapVocab(WebApplication app, string path, string atom, string noun,
    ...
    static void MapFrequencies(WebApplication app)
    {
        ...
        app.MapPost("/api/icu/formulary/frequencies", (CreateFrequencyRequest req, ClaimsPrincipal user, AuroraDb db) =>
        {
            ...
            if (!System.Text.RegularExpressions.Regex.IsMatch(value, @"^[a-zA-Z0-9][a-zA-Z0-9 _/\-]{1,39}$"))
                return ApiError.BadRequest("value must be 2-40 characters: letters, digits, spaces, '-', '_' or '/' (it appears verbatim on orders, e.g. 'with meals')");
