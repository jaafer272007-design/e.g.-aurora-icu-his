...
class AuroraDb(DbContextOptions<AuroraDb> options) : DbContext(options)
{
    ...
    /* Imaging Catalogue (the lab catalogue's imaging counterpart): coded
       study DEFINITIONS each hospital manages from /config — distinct
       from ImagingStudies below, which are performed studies (reports) */
    public DbSet<ImagingStudyDefRow> ImagingCatalog => Set<ImagingStudyDefRow>();
